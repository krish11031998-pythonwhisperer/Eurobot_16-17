...
int pos1 = 130;    // variable to store the servo position
int pos2 = 130;
int pos3 = 100;
int pos4 = 130;
...
void drive(int speed, int turnspeed, float wd1, float wd2, int acceleration) {
  ...
  Serial.println(tempEncoder2);
  float sikis1 = tempEncoder1;
  float sikis2 = tempEncoder2;
  if (sikis1 - sikis2 > 1) {
  ...
  if (tempEncoder1 && tempEncoder2 < 0.2) {
    return;
  }
  //  if ( speed = ! 0) {
  //    if (sikis1 >= 0) {
  ...
  //    gg = 1;
  //  }
  drive((-speed / 2), (-turnspeed) / 2, tempEncoder1, tempEncoder2, -1);
}
...
int ping()
{
  ...
  delay(10);
  digitalWrite(trigpin, LOW);       //Used to read in the pulse that is being sent by the MaxSonar device. //Pulse Width
...
int ping1()
{
  ...
  delay(10);
  digitalWrite(trigpin1, LOW);      //Used to read in the pulse that is being sent by the MaxSonar device. //Pulse Width
...
void loop() {
  ...
  myservo2.write(0);
      ...
      autostop();
  while (millis() < 90000) {
    if (switchSide) {  // right side start code
      drive(-10, 0, 69, 69, 1);
      delay(100);
      ...
      myservo4.write(pos4);
      delay(100);
      drive(7.5, 0, 6.5, 6.5, 0);
      delay(100);
      drive(0, 20, 16.3, -16.3, 0);
      delay(100);
      pos = 50;
      myservo.write(pos);
      pos3 = 20;
